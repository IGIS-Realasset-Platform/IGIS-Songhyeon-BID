...
export default function LegacyApp() {
 // Hardcode base for legacy app routing
 const BASE = import.meta.env.BASE_URL + (import.meta.env.BASE_URL.endsWith('/') ? 'sbd-bid/' : '/sbd-bid/');
 
 const getPage = () => {
   const base = BASE.endsWith('/') ? BASE.slice(0, -1) : BASE;
   let path = window.location.pathname.replace(base, '').replace(/^\//, '');
   if (path.startsWith('sbd-bid')) {
     path = path.replace(/^sbd-bid\/?/, '');
   }
