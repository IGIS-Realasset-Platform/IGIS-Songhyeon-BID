...
const activityDescription = (item) => {
  ...
  if (item.action === 'task_reverted') return `미착수로 변경${item.payload?.reason ? ` · ${item.payload.reason}` : ''}`;
...
export default function SonghyeonTaskDetailDrawer({ task, onClose, onBackdropClick, onSaved, canArchive = false, onArchiveRequest, forceReadOnly = false }) {
  ...
  return (
    <div className="fixed inset-0 z-[100000] overflow-hidden pointer-events-none" data-task-detail-overlay data-pmo-task-detail-overlay>
      <button type="button" aria-label="업무 상세 닫기" onClick={onBackdropClick || onClose} className="absolute inset-0 cursor-pointer bg-black/10 pointer-events-auto" />
