...
export default function SonghyeonTaskFeed() {
  ...
  return (
    <section className="w-full" aria-label="업무 피드 게시판">
            ...
            return (
              ...
              >
                <button type="button" onClick={() => setExpanded((current) => ({ ...current, [post.id]: !isExpanded }))} className="grid w-full min-w-[1080px] cursor-pointer grid-cols-[116px_90px_126px_minmax(260px,1fr)_100px_118px_72px_82px_68px_76px] items-center px-5 py-4 text-[13px] text-[#A1A1AA]">
                  ...
                  <span className="flex items-center gap-2 text-left"><Avatar profile={{ name: post.authorName, photoPath: post.authorPhotoPath }} /><span className="min-w-0 truncate font-bold text-[#E5E5E5]">{post.authorName || '-'}</span></span>
      ...
      {editingPost ? <div className="fixed inset-0 z-[110000] grid place-items-center bg-black/70 p-6" role="dialog" aria-modal="true" aria-label="업무 메시지 수정"><div className="max-h-[calc(100vh-48px)] w-[940px] max-w-full overflow-y-auto rounded-[20px] border border-[#444] bg-[#202020] p-2 shadow-2xl"><div className="flex justify-end p-2"><button type="button" aria-label="닫기" onClick={() => setEditingPost(null)} className="grid h-9 w-9 place-items-center rounded-[8px] text-[#86868B] hover:bg-[#333] hover:text-white"><X size={18} /></button></div><SonghyeonTaskFeedWriteBox actor={actor} options={options} tasks={options.tasks || []} isReadOnly={isReadOnly} initialPost={editingPost} onSaved={async () => { setEditingPost(null); await refresh(); }} onCancel={() => setEditingPost(null)} /></div></div> : null}
