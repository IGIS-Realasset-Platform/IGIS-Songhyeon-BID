...
function PlanAccordion({ plan, open, onToggle, onMap, highlighted = false }) {
  return (
    <article data-governance-card="plan" id={`plan-record-${plan.id}`} tabIndex={-1} className={`${highlighted ? 'border-[#78a5c4] bg-[#2d373e] ring-1 ring-[#78a5c4]' : 'border-[#40505b] bg-[#252b2f]'} overflow-hidden rounded-[10px] border border-l-[3px] border-l-[#5f8299] shadow-[0_3px_12px_rgba(0,0,0,0.12)] outline-none transition-[background-color,border-color,box-shadow] duration-300`}>
      <button type="button" onClick={onToggle} className="flex w-full items-start justify-between gap-3 p-3.5 text-left hover:bg-[#2b3338]">
        <div className="min-w-0"><div className="flex max-w-full flex-wrap items-start gap-2"><span className="max-w-full break-words rounded-[5px] border border-[#526674] bg-[#2a353d] px-2 py-1 text-[14px] font-bold leading-[17px] text-[#b7cedd]">{display(plan.type)}</span></div><h4 className="mt-2 text-[16px] font-bold leading-5 text-[#eeeeef]">{plan.title}</h4><p className="mt-1.5 text-[14px] leading-[18px] text-[#a0a3a7]">{display(plan.scope)}</p></div>
        {open ? <ChevronDown size={17} className="mt-1 shrink-0 text-[#9ec1d8]" /> : <ChevronRight size={17} className="mt-1 shrink-0 text-[#7694a8]" />}
      </button>
      {open && <div className="border-t border-[#40505b] bg-[#22282c] p-4"><dl className="grid grid-cols-1 gap-x-5 gap-y-4 min-[700px]:grid-cols-2"><Field label="공간범위" wide>{display(plan.scope)}</Field><Field label="시행일">{display(plan.effective_from)}</Field><Field label="상태">{display(plan.status)}</Field><Field label="적용 후보">{display(plan.candidate_relevance)}</Field><Field label="확인일">{formatDate(plan.checked_at)}</Field><Field label="핵심 내용" wide>{plan.content}</Field><Field label="송현 영향" wide>{plan.impact}</Field><Field label="추가 확인" wide>{plan.open_issue}</Field></dl><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => onMap(plan)} className="inline-flex min-h-8 items-center gap-1.5 rounded-[7px] border border-[#506879] bg-[#29343c] px-2.5 py-1.5 text-[14px] font-bold text-[#b2d0e4]"><MapPin size={13} /> 관련 공간 지도</button>{sourceLink(plan.source_url)}</div></div>}
    </article>
  );
}
...
function OrganizationAccordion({ organization, open, onToggle }) {
  return (
    <article data-governance-card="organization" className="overflow-hidden rounded-[10px] border border-l-[3px] border-[#514858] border-l-[#9b7eaa] bg-[#2b282e] shadow-[0_3px_12px_rgba(0,0,0,0.12)]">
      <button type="button" onClick={onToggle} className="flex w-full items-start justify-between gap-3 p-3.5 text-left hover:bg-[#332e36]">
        <div className="min-w-0"><div className="flex items-center gap-2"><Users size={15} className="shrink-0 text-[#b99bc7]" /><span className="text-[14px] font-medium text-[#b2a7b7]">{display(organization.scope)}</span></div><h4 className="mt-2 text-[16px] font-bold leading-5 text-[#eeeeef]">{organization.name}</h4><p className="mt-1.5 text-[14px] leading-[18px] text-[#a19da4]">{display(organization.identity)}</p></div>{open ? <ChevronDown size={17} className="mt-1 shrink-0 text-[#c2a6cf]" /> : <ChevronRight size={17} className="mt-1 shrink-0 text-[#937c9d]" />}
      </button>
      {open && <div className="border-t border-[#514858] bg-[#272329] p-4"><dl className="grid grid-cols-1 gap-x-5 gap-y-4 min-[700px]:grid-cols-2"><Field label="확인 역할" wide>{organization.identity}</Field><Field label="연결 가능성" wide>{organization.relevance}</Field><Field label="활동 요약" wide>{organization.activity_summary}</Field><Field label="확인일">{formatDate(organization.checked_at)}</Field><Field label="근거">{sourceLink(organization.source_url)}</Field></dl><div className="mt-4 border-t border-[#49404e] pt-4"><p className="text-[14px] font-bold text-[#aaa0ae]">확인된 활동 {organization.activities?.length || 0}건</p><div className="mt-2 space-y-2">{(organization.activities || []).map((activity, index) => <div key={`${activity.title}-${activity.date}-${index}`} className="rounded-[8px] border border-[#453d49] bg-[#242126] p-3"><div className="flex items-start justify-between gap-3"><p className="text-[15px] font-bold leading-[18px] text-[#d8d5da]">{activity.title}</p><span className="shrink-0 text-[14px] tabular-nums text-[#8d858f]">{formatDate(activity.date)}</span></div><p className="mt-1.5 text-[14px] leading-[18px] text-[#a09aa2]">{activity.summary}</p><div className="mt-2">{sourceLink(activity.source_url, '활동 원문')}</div></div>)}</div></div></div>}
    </article>
  );
}
...
export function InstitutionsCommunityWorkspace({ institutionData, boundaryData, integratedData, marketData, focusPlanId = '' }) {
  ...
  return (
    <div data-institutions-community-workspace className="h-full min-h-0 overflow-y-auto bg-[#1f2021] px-[clamp(24px,4vw,72px)] py-5 text-[#d6d6d8]">
      <div data-institutions-community-content className="mx-auto w-full max-w-[1440px]">
        ...
        <div className="grid grid-cols-1 gap-5 min-[981px]:grid-cols-2">
          <section data-governance-plans-panel className="min-w-0 rounded-[12px] border border-[#40515d] bg-[#20272b] shadow-[0_10px_30px_rgba(0,0,0,0.16)]">
            <header className="sticky top-0 z-10 flex min-h-[58px] items-center rounded-t-[12px] border-b border-t-2 border-[#40515d] border-t-[#6f98b2] bg-[#28333a] px-5"><h3 className="text-[17px] font-bold text-[#dceaf2]">계획·제도 {plans.length}건</h3></header>
            <div className="space-y-6 p-4">{planGroups.map(([year, rows]) => <div key={year} data-governance-plan-group><div className="mb-3 flex items-center gap-3 rounded-[8px] border border-[#3f5260] bg-[#25323a] px-3 py-2"><span className="text-[16px] font-black tabular-nums text-[#b7d3e5]">{year}</span><span className="h-px flex-1 bg-[#4a6374]" /><span className="rounded-full bg-[#31434f] px-2 py-0.5 text-[14px] font-bold text-[#a9c6d8]">{rows.length}건</span></div><div className="space-y-3">{rows.map((plan) => <PlanAccordion key={plan.id} plan={plan} open={openPlans.has(plan.id)} onToggle={() => toggleSet(setOpenPlans, plan.id)} onMap={setMapPlan} highlighted={focusPlanId === plan.id} />)}</div></div>)}</div>
          </section>

          <section data-governance-organizations-panel className="min-w-0 rounded-[12px] border border-[#51475a] bg-[#272329] shadow-[0_10px_30px_rgba(0,0,0,0.16)]">
            <header className="sticky top-0 z-10 flex min-h-[58px] items-center rounded-t-[12px] border-b border-t-2 border-[#51475a] border-t-[#a486b3] bg-[#302a34] px-5"><h3 className="text-[17px] font-bold text-[#eadff0]">공동체·활동 기록 · {organizations.length}곳</h3></header>
            <div className="space-y-6 p-4">{organizationGroups.map(({ group, rows }) => <div key={group} data-governance-organization-group><div className="mb-3 flex items-center gap-3 rounded-[8px] border border-[#574b5e] bg-[#302a34] px-3 py-2"><span className="text-[15px] font-black text-[#d0b8da]">{group}</span><span className="h-px flex-1 bg-[#66566e]" /><span className="rounded-full bg-[#403647] px-2 py-0.5 text-[14px] font-bold text-[#c3aecd]">{rows.length}곳</span></div><div className="space-y-3">{rows.map((organization) => <OrganizationAccordion key={organization.id} organization={organization} open={openOrganizations.has(organization.id)} onToggle={() => toggleSet(setOpenOrganizations, organization.id)} />)}</div></div>)}</div>
          </section>
        </div>

        <section className="mt-5 grid grid-cols-1 gap-5 min-[981px]:grid-cols-2">
