  ...
  return (
    <WorkspacePageFrame>
        ...
        actions={(
          <div className="flex items-center gap-2">
            <div className="flex h-[36px] items-center rounded-[9px] border border-[#3c3c3c] bg-[#272726] p-1" aria-label="이용 현황 보기">
              <button type="button" onClick={() => changeView('all')} aria-pressed={!isMemberView} className={`h-[26px] cursor-pointer rounded-[6px] px-3 text-[11px] font-bold transition-colors ${!isMemberView ? 'bg-[#6f9fc7]/20 text-[#9cc4e6]' : 'text-[#86868B] hover:text-white'}`}>전체 이용</button>
              <button type="button" onClick={() => changeView('members')} aria-pressed={isMemberView} className={`h-[26px] cursor-pointer rounded-[6px] px-3 text-[11px] font-bold transition-colors ${isMemberView ? 'bg-[#6f9fc7]/20 text-[#9cc4e6]' : 'text-[#86868B] hover:text-white'}`}>TF 인원별</button>
            </div>
      ...
      <section className="mt-3 overflow-hidden rounded-[20px] border border-[#3c3c3c] bg-[#272726]" aria-labelledby="analytics-recent-title">
        <div className="flex h-[52px] items-center justify-between border-b border-[#3c3c3c] px-5"><h2 id="analytics-recent-title" className="text-[14px] font-bold text-white">최근 이용</h2><span className="text-[10px] text-[#686868]">게스트 익명 · TF 회원 프로필 기준 · 최대 400일 보관</span></div>
        ...
        <div className="grid grid-cols-[360px_minmax(0,1fr)] items-start gap-3">
          <section className="overflow-hidden rounded-[20px] border border-[#3c3c3c] bg-[#272726]" aria-labelledby="member-analytics-list-title">
            <div className="flex h-[56px] items-center justify-between border-b border-[#3c3c3c] px-5">
              <div><h2 id="member-analytics-list-title" className="text-[14px] font-bold text-white">TF 인원별 이용</h2><p className="mt-0.5 text-[9px] text-[#686868]">선택 기간 내 로그인 활동</p></div>
              <span className="text-[10px] font-bold text-[#9cc4e6]">{number(members.length)}명</span>
            </div>
            <div className="divide-y divide-[#3c3c3c]/70">
              {members.map((member) => {
                ...
                return (
                  <button key={member.id} type="button" onClick={() => setSelectedMemberId(member.id)} aria-pressed={selected} className={`w-full cursor-pointer px-4 py-3 text-left transition-colors ${selected ? 'bg-[#6f9fc7]/12' : 'hover:bg-white/[0.025]'}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><strong className={`truncate text-[13px] ${selected ? 'text-[#b7d5ee]' : 'text-[#E5E5E5]'}`}>{memberName(member)}</strong><span className="shrink-0 text-[13px] font-bold tabular-nums text-[#9cc4e6]">{number(member.views)} <small className="text-[9px] font-normal text-[#686868]">조회</small></span></div><p className="mt-0.5 truncate text-[10px] text-[#86868B]">{[memberGroup(member), member.title].filter(Boolean).join(' · ') || '-'}</p></div>
                    </div>
                    <div className="mt-2 grid grid-cols-3 gap-2 pl-[52px] text-[9px] text-[#686868]"><span>세션 <b className="text-[#A1A1AA]">{number(member.sessions)}</b></span><span>활동일 <b className="text-[#A1A1AA]">{number(member.activeDays)}</b></span><span className="truncate text-right" title={dateTime(member.lastViewedAt)}>{member.lastViewedAt ? dateTime(member.lastViewedAt) : '최근 이용 없음'}</span></div>
                  </button>
                );
              })}
              {!loading && members.length === 0 && <div className="px-5 py-12 text-center text-[12px] text-[#686868]">표시할 TF 인원이 없습니다.</div>}
            </div>
            <div className="border-t border-[#3c3c3c] bg-[#222]/70 px-4 py-3 text-[9px] leading-4 text-[#777]">
          ...
          <div className="min-w-0 space-y-3">
            <section className="rounded-[20px] border border-[#3c3c3c] bg-[#272726] p-5" aria-labelledby="member-detail-title">
              <div className="flex items-center justify-between">
                <div className="flex min-w-0 items-center gap-3"><SonghyeonMemberAvatar name={memberName(selectedMember)} photoPath={selectedMember?.photoPath} className="h-12 w-12 border border-white/10" /><div className="min-w-0"><h2 id="member-detail-title" className="text-[14px] font-bold text-white">TF 인원 상세 트래킹</h2><p className="mt-1 truncate text-[12px] text-[#A1A1AA]"><b className="text-[#E5E5E5]">{memberName(selectedMember)}</b>{memberGroup(selectedMember) ? ` · ${memberGroup(selectedMember)}` : ''}</p></div></div>
                <span className="text-[10px] text-[#686868]">최근 이용 {dateTime(memberSummary.lastViewedAt)}</span>
              </div>
            ...
            <div className="grid grid-cols-[1.25fr_1fr] gap-3">
              <section className="rounded-[20px] border border-[#3c3c3c] bg-[#272726] p-5" aria-labelledby="member-daily-title">
                <div className="flex items-center justify-between"><h3 id="member-daily-title" className="text-[14px] font-bold text-white">일별 이용 추이</h3><span className="text-[10px] text-[#686868]">조회·세션</span></div>
                <div className="mt-5 flex h-[176px] items-end gap-1" role="img" aria-label={`${memberName(selectedMember)}의 최근 ${days}일 조회수`}>
                  {memberDetail.daily.map((item, index) => {
                    ...
                    return <div key={item.date} className="group relative flex min-w-0 flex-1 flex-col items-center justify-end" title={`${item.date} · 조회 ${number(item.views)} · 세션 ${number(item.sessions)}`}><span className="pointer-events-none absolute bottom-[calc(100%+6px)] z-10 hidden whitespace-nowrap rounded-[6px] border border-[#454545] bg-[#1c1c1e] px-2 py-1 text-[10px] text-[#C7C7CC] shadow-lg group-hover:block">조회 {number(item.views)} · 세션 {number(item.sessions)}</span><div className="w-full max-w-[16px] rounded-t-[4px] bg-[#6f9fc7]/70" style={{ height }} /><span className="mt-2 h-3 truncate text-[8px] text-[#686868]">{showLabel ? shortDate(item.date) : ''}</span></div>;
                  })}
                  {!loading && memberDetail.daily.length === 0 && <div className="grid h-full w-full place-items-center text-[12px] text-[#686868]">이용 기록이 없습니다.</div>}
                </div>
              </section>

              <section className="overflow-hidden rounded-[20px] border border-[#3c3c3c] bg-[#272726]" aria-labelledby="member-pages-title">
                <div className="flex h-[52px] items-center justify-between border-b border-[#3c3c3c] px-4"><h3 id="member-pages-title" className="text-[14px] font-bold text-white">페이지별 이용</h3><span className="text-[10px] text-[#686868]">상위 {Math.min(memberDetail.pages.length, 7)}개</span></div>
                <div className="divide-y divide-[#3c3c3c]/70">{memberDetail.pages.slice(0, 7).map((page, index) => <div key={page.path} className="grid grid-cols-[20px_1fr_auto] items-center gap-2 px-4 py-2.5"><span className="font-mono text-[9px] text-[#686868]">{String(index + 1).padStart(2, '0')}</span><div className="min-w-0"><div className="truncate text-[11px] font-bold text-[#E5E5E5]">{PAGE_LABELS[page.path] || page.path}</div><div className="mt-0.5 truncate text-[8px] text-[#686868]">세션 {number(page.sessions)}</div></div><span className="text-[12px] font-bold tabular-nums text-[#9cc4e6]">{number(page.views)}</span></div>)}{!loading && memberDetail.pages.length === 0 && <div className="px-4 py-12 text-center text-[11px] text-[#686868]">집계된 페이지가 없습니다.</div>}</div>
              </section>
            </div>

            <section className="overflow-hidden rounded-[20px] border border-[#3c3c3c] bg-[#272726]" aria-labelledby="member-recent-title">
              <div className="flex h-[52px] items-center justify-between border-b border-[#3c3c3c] px-5"><h3 id="member-recent-title" className="text-[14px] font-bold text-white">최근 활동</h3><span className="text-[10px] text-[#686868]">최근 {Math.min(memberDetail.recent.length, 10)}건</span></div>
              <div className="grid grid-cols-2 divide-x divide-[#3c3c3c]/70">{memberDetail.recent.slice(0, 10).map((entry, index) => <div key={`${entry.path}-${entry.viewedAt}-${index}`} className="flex items-center justify-between gap-3 border-b border-[#3c3c3c]/60 px-5 py-3"><div className="min-w-0"><div className="truncate text-[11px] font-bold text-[#E5E5E5]">{PAGE_LABELS[entry.path] || entry.path}</div><div className="mt-0.5 truncate font-mono text-[8px] text-[#686868]">{entry.path}</div></div><time className="shrink-0 text-[9px] text-[#86868B]">{dateTime(entry.viewedAt)}</time></div>)}{!loading && memberDetail.recent.length === 0 && <div className="col-span-2 px-5 py-10 text-center text-[12px] text-[#686868]">최근 활동 기록이 없습니다.</div>}</div>
            </section>
          </div>
        </div>
      )}
