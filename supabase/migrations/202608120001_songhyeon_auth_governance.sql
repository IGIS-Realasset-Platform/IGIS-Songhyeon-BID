...
-- Pre-auth email step: returns only the matched active member's display name.
...
as $$
  select m.staff_name, (m.auth_id is null) from public.songhyeon_members m
  where m.email = lower(trim(candidate_email)) and m.is_active
  limit 1;
$$;
...
as $$
declare claimed public.songhyeon_members;
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  if not exists (
    select 1 from public.songhyeon_auth_settings s
    where s.singleton and s.access_code_hash = extensions.crypt(candidate_access_code, s.access_code_hash)
  ) then raise exception 'INVALID_ACCESS_CODE'; end if;
  update public.songhyeon_members m set auth_id = auth.uid(), updated_at = now()
  where m.email = lower(auth.jwt()->>'email') and m.is_active
    and (m.auth_id is null or m.auth_id = auth.uid())
  returning m.* into claimed;
  if claimed.id is null then raise exception 'MEMBERSHIP_NOT_FOUND'; end if;
  return claimed;
end; $$;
...
revoke all on function public.check_songhyeon_member_email(text) from public;
revoke all on function public.claim_songhyeon_membership(text) from public;
