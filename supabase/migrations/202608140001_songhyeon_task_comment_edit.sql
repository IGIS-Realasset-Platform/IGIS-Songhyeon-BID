-- Songhyeon task discussion editing only.
-- Comment and reply authors can edit their own body through controlled RPCs.

alter table public.songhyeon_task_comments
  add column if not exists updated_at timestamptz not null default now(),
  add column if not exists edited_at timestamptz;

alter table public.songhyeon_task_comment_replies
  add column if not exists updated_at timestamptz not null default now(),
  add column if not exists edited_at timestamptz;
...
begin
  ...
  if not public.is_songhyeon_member(current_actor_id) then
  ...
  if current_comment.author_id <> current_actor_id then
...
begin
  ...
  if current_reply.author_id <> current_actor_id then
...
-- Keep direct table UPDATE unavailable. The RPCs preserve authorship and scope.
revoke update on table public.songhyeon_task_comments from authenticated;
revoke update on table public.songhyeon_task_comment_replies from authenticated;
